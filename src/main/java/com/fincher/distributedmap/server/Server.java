...
public class Server implements Closeable {
    ...
    private void handleRequestKeyLock(RequestKeyLock msg, String channelId) throws ChannelException {
        ...
        if (info == null) {
            ...
            synchronized(info) {
                if (msg.getLatestTransId() != info.getTransactionId()) {
                    response.setLockAcquired(false);
                } else {
        ...
        ServerToClientMessage wrapper = ServerToClientMessage.newBuilder().setRequestKeyLockResponse(response.build()).build();
    ...
    private void handleRequestMapLock(RequestMapLock req, String channelId) throws ChannelException {
        ...
        ServerToClientMessage wrapper = ServerToClientMessage.newBuilder().setRequestMapLockResponse(response.build()).build();
        Utilities.sendMessage(channel, channelId, wrapper);
    }
    ...
    private void handleRequestMapChange(RequestMapChange req, String channelId) throws ChannelException {
        ...
        if (info == null) {
            ...
            synchronized(info) {
                if (info.getTransactionId() + 1 == transaction.getId()) {
                    response.setUpdateSuccess(false);
        ...
        ServerToClientMessage wrapper = ServerToClientMessage.newBuilder().setRequestMapChangeResponse(response.build()).build();
        Utilities.sendMessage(channel, channelId, wrapper);
    }
    ...
    private final void updateClient(String mapName, String channelId) throws ChannelException {
        ...
        synchronized(info) {
            ...
            if (client.mapTransId != info.getTransactionId()) {
                ...
                ServerToClientMessage msg = ServerToClientMessage.newBuilder().setClientTransactionUpdate(builder.build()).build();
